...
def main():
    ...
    upload_stats = upload_csv_to_gcs(
        csv_url=csv_url, bucket_name=config.GCS_BUCKET, file_id=file_id
    )

    load_stats = load_csv_to_raw_layer(
        gcs_uri=gcs_uri, raw_table_id=raw_table_id
    )

    clean_stats = clean_raw_data(
        raw_table_id=raw_table_id, clean_table_id=clean_table_id
    )
