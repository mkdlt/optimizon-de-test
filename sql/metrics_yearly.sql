with most_popular as (
  select
    date_trunc(purchased_date, year) as period,
    sku
  from {{clean_table_id}} 
  group by 1, 2
...
json_objects as (
  select
    date_trunc(purchased_date, year) as period,
    json_object(
      'period', max(date_trunc(purchased_date, year)),
  ...
  inner join most_popular
    on date_trunc(purchased_date, year) = most_popular.period
  inner join least_popular
    on date_trunc(purchased_date, year) = least_popular.period
  group by 1
  order by 1
)
